...
import { AlertCircle, CheckCircle2, Clock, XCircle, Plus, LogOut, Search, User, Activity } from 'lucide-react';
...
class ApiService {
  ...
  async register(name: string, email: string, password: string, role: Role = 'USER'): Promise<AuthResponse> {
...
const StatusBadge: React.FC<{ status: TicketStatus }> = ({ status }) => {
  const statusConfig: Record<TicketStatus, { bg: string; text: string; icon: React.ComponentType<any> }> = {
...
const CreateTicketModal: React.FC<{ onClose: () => void; onSuccess: () => void }> = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      setError('Error al crear el ticket. Verifica los IDs.');
    } finally {
      setLoading(false);
    }
  };
